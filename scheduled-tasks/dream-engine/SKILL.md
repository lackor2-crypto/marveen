...
SQL minta:
```bash
dbq "SELECT agent_id, content, keywords FROM memories WHERE created_at > CAST(strftime('%s', 'now', '-24 hours') AS INTEGER) AND category IN ('hot','warm') ORDER BY agent_id, created_at"
```
...
dbq "SELECT id, content, created_at, updated_at FROM memories WHERE category='hot' AND COALESCE(updated_at, created_at) < CAST(strftime('%s', 'now', '-7 days') AS INTEGER)"
```
