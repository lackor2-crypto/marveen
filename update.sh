...
if [ -d "$SEED_SCHED_DIR" ]; then
  if [ -z "$MAIN_AGENT_ID" ]; then
    ...
    for tpl in "$SEED_SCHED_DIR"/*/; do
      ...
      mkdir -p "$target"
      # find, not a top-level glob: a task template may carry subfolders
      # (bumblebee-hygiene-scan/threat-intel), which a `for f in "$tpl"*`
      # loop drops without a word. Same loop as install-linux.sh's.
      while IFS= read -r -d '' f; do
        rel="${f#"$tpl"}"
        mkdir -p "$target/$(dirname "$rel")"
        # {{WEB_PORT}} and {{PROJECT_ROOT}} belong here too: install-linux.sh
        # substitutes them in its seeding loops, and a task seeded by THIS path
        # used to keep the literal placeholder in its URLs. Same placeholder set
        # on both paths, or a template silently means different things depending
        # on which script created the copy.
        sed -e "s/{{MAIN_AGENT_ID}}/$MAIN_AGENT_ID/g" \
            -e "s/{{BOT_NAME}}/$BOT_NAME/g" \
            -e "s/{{OWNER_NAME}}/$OWNER_NAME/g" \
            -e "s|{{INSTALL_DIR}}|$INSTALL_DIR|g" \
