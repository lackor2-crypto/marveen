...
def known_agent_id(cwd):
    ...
    """
    return _known_or_none(agent_id_from_cwd(cwd))


def known_agent_id_from_payload(payload):
    """The strict writer twin of agent_id_from_payload(): the session-stable
    identity (transcript_path -> MARVEEN_AGENT_ID -> cwd), but None when the
    result does not name a REAL agent. Writers use this so a row is filed under
    the session's own agent even after its shell cd'd elsewhere, and never under
    an invented one (see known_agent_id)."""
    return _known_or_none(agent_id_from_payload(payload))


def _known_or_none(guess):
    if not guess:
        return None
