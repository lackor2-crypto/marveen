...
## Mikor használd
Boss telefonrol/mobilrol akarja elerni a Marveen dashboardot (localhost:3420).
...
1. Tailscale telepites WSL-ben (sudo -> Boss futtatja): `curl -fsSL https://tailscale.com/install.sh | sh`
2. Belepes (Boss, browser consent): `sudo tailscale up` -> a linket megnyitja, engedelyezi a fiokjaval.
...
- A telepites sudo + a `tailscale up` Boss fiokja -> ezek Boss-lepesek (package_install + browser consent). Ne probald headless.
- LAN alternativa (azonos WiFi): WEB_HOST=0.0.0.0 + Windows `netsh interface portproxy` a WSL-IP-re + tuzfal-szabaly (admin). Torékeny: a WSL belso IP ujrainditaskor valtozik, csak HTTP, csak otthoni halo. Csak ha Boss ezt keri.
- A tokent (`store/.dashboard-token`) SOSE ird chatbe. Boss a gepen olvassa ki, vagy device key-t adunk.
