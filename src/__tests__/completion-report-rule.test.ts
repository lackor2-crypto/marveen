// The mandatory "always send a completion signal on the owner's channel" rule
// must reach EVERY agent, not just new ones. 2026-09-16: after landing #290 the
// completion signal was skipped ("the owner did not ask for feedback"), so the
// owner could not tell the delegated work was done -- a silently finished task
// is invisible from the owner's channel.
//
// What this test protects (same contract as no-live-tree-rule.test.ts):
...
import { ensureCompletionReportSection } from '../web/agent-scaffold.js'

const THROWAWAY = 'zz-completion-report-probe'
...
describe('ensureCompletionReportSection', () => {
  it('appends the rule to an existing agent CLAUDE.md', () => {
    const path = seed('# zz-completion-report-probe\n\nSajat tartalom.\n')
    ensureCompletionReportSection(THROWAWAY)
    const out = readFileSync(path, 'utf-8')
    expect(out).toContain('FELADAT VEGEN KOTELEZO KESZ-JELZES A TULAJDONOSNAK')
    expect(out).toContain('kesz-jelzes kotelezo')
  ...
  it('is idempotent: no duplicate block and no rewrite on the second call', () => {
    ...
    const occurrences = second.split('BEGIN GENERATED: completion-report-rule').length - 1
  ...
  it('does nothing when the agent has no CLAUDE.md (fresh install)', () => {
    ...
    expect(() => ensureCompletionReportSection(THROWAWAY)).not.toThrow()
  ...
  it('skips the main agent (its rule is tracked template text, not a generated block)', () => {
    ...
    ensureCompletionReportSection(MAIN_AGENT_ID)
    expect(readFileSync(repoClaudeMd, 'utf-8')).toEqual(before)
    expect(before).not.toContain('BEGIN GENERATED: completion-report-rule')
  })

  it('reports WHY nothing was written, so zero is never ambiguous', () => {
    expect(ensureCompletionReportSection('zz-no-such-agent-at-all')).toBe('no-file')
    expect(ensureCompletionReportSection(MAIN_AGENT_ID)).toBe('skipped-main')

    seed('# zz-completion-report-probe\n\nSajat tartalom.\n')
    expect(ensureCompletionReportSection(THROWAWAY)).toBe('written')
    expect(ensureCompletionReportSection(THROWAWAY)).toBe('current')
  })
})
...
describe('the rule reaches fresh installs and the whole fleet', () => {
  it('the main-agent CLAUDE.md template carries the rule', () => {
    ...
    expect(text).toContain('FELADAT VÉGÉN KÖTELEZŐ')
    expect(text).toContain('ensureCompletionReportSection')
  })
})
