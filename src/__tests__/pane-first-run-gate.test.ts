...
describe('first-run gate wiring contracts', () => {
  ...
  it('startAgentProcess stamps per-project trust in the config root the session boots from', () => {
    ...
    // before the session spawns), not about the exact tmux argument list or the
    // launch target (`null`, or `startTarget` once agents can own their OS user).
    const launchIdx = AGENT_PROCESS.search(/runTmux\((?:null|startTarget), \['new-session',[^\]]*'-s', session, cmd\]/)
