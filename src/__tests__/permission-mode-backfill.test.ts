...
import { decidePermissionMode, ensureUserPermissionMode } from '../web/agent-scaffold.js'
...
describe('a lanc tenylegesen be van kotve', () => {
  ...
  it('boot-kor egyszer lefut, a tulaj sajat ~/.claude/settings.json-jara', () => {
    expect(WEB_TS).toContain('if (ensureUserPermissionMode())')
    expect(SCAFFOLD).toMatch(/return join\(homedir\(\), '\.claude', 'settings\.json'\)/)
  })

  it('a macOS telepito is beirja (friss telepites macen se kerdezzen)', () => {
    const mac = readFileSync(join(ROOT, 'install-macos.sh'), 'utf8')
    expect(mac).toMatch(/defaultMode"?\]?\s*=\s*"bypassPermissions"/)
  })
})

describe('ensureUserPermissionMode a lemezen', () => {
  const dir = mkdtempSync(join(tmpdir(), 'marveen-permmode-'))
  const f = join(dir, 'sub', 'settings.json')

  it('hianyzo fajlnal letrehozza a sablon ertekevel', () => {
    expect(ensureUserPermissionMode(f)).toBe(true)
    expect(JSON.parse(readFileSync(f, 'utf8')).permissions.defaultMode).toBe('bypassPermissions')
  })

  it('masodszorra nem ir (idempotens)', () => {
    expect(ensureUserPermissionMode(f)).toBe(false)
  })

  it('a tulaj "auto" dontesehez nem nyul', () => {
    writeFileSync(f, JSON.stringify({ permissions: { defaultMode: 'auto' }, model: 'x' }))
    expect(ensureUserPermissionMode(f)).toBe(false)
    expect(JSON.parse(readFileSync(f, 'utf8')).permissions.defaultMode).toBe('auto')
  })

  it('olvashatatlan fajlt nem tapos le', () => {
    writeFileSync(f, '{ ez nem json')
    expect(ensureUserPermissionMode(f)).toBe(false)
    expect(readFileSync(f, 'utf8')).toBe('{ ez nem json')
  })
})
