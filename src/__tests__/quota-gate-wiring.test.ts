...
describe('schedule-runner wiring', () => {
  ...
  it('gates before the pre-check, so a deferred task never spawns its script', () => {
    // A horgony await-fuggetlen: nalunk runPreCheck() async (`await
    // runPreCheck(task)`), az upstreamben szinkron volt. A rogzitett invarians
    // a SORREND, nem a hivas alakja. A nyers indexOf -1-et ad, ha nem talal --
    // az NEMAN atmenne a toBeLessThan-en, ezert eloszor a megletet allitjuk.
    const gateAt = RUNNER_SRC.indexOf("if (quota.action === 'defer')")
    const preCheckAt = RUNNER_SRC.search(/const cronPc = (await )?runPreCheck\(task\)/)
    expect(gateAt, 'quota-gate anchor not found in schedule-runner.ts').toBeGreaterThan(-1)
    expect(preCheckAt, 'pre-check anchor not found in schedule-runner.ts').toBeGreaterThan(-1)
    expect(gateAt).toBeLessThan(preCheckAt)
  })
})
