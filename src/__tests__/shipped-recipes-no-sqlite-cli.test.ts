...
describe('both seeding paths substitute the same placeholders', () => {
  ...
  function placeholdersAfter(src: string, anchor: string): Set<string> {
    ...
    // A sed-blokk utolso sora a kiiras. Nem kotjuk a hurok fajtajahoz:
    // a top-level glob (`$(basename "$f")`) es a rekurziv find (`$rel`)
    // ugyanugy ide er ki -- ez utobbi az elo valtozat mindket vetesi uton.
    const end = src.indexOf('"$f" > "$target/', sed)
