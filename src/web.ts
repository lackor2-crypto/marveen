...
export function startWebServer(port = 3420): http.Server {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (await tryHandleWorkbenchAgent(routeCtx)) return
  ...
  if (!hookDecision.register) {
    ...
    try {
      ...
      for (const agentName of [MAIN_AGENT_ID, ...listAgentNames()]) {
        ...
        // ...es az "egy hiba = egy kartya, azonnal keszre" doktrina (Boss,
        // 2026-09-07): egy hibat nem darabolunk szet tobb kartyara, nem tolunk
        // at masik kartya ala, es nem hagyunk felbe egy kartyat.
        ...
        // ...es a "soha ne dolgozz kozvetlenul az elo tree-ben" szabaly (Boss,
        // 2026-09-12): minden munka izolalt worktree-ben + land-pr.sh; a fo
        // checkout main-jere kozvetlen commit megallitja az auto-deploy-t.
        const noLiveTree = ensureNoLiveTreeSection(agentName)
        if (noLiveTree === 'written' && !askBackWritten.includes(agentName)) askBackWritten.push(agentName)
        if (noLiveTree === 'unreadable' && !askBackUnreadable.includes(agentName)) askBackUnreadable.push(agentName)
        // ...es a "feladat vegen kotelezo kesz-jelzes a tulajdonosnak" szabaly
        // (2026-09-16): egy csendben befejezett feladat kivulrol lathatatlan,
        // a "ne kelljen figyelned" csak a HALADASRA vonatkozik, a keszre soha.
