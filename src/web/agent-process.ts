...
import { writeAgentSettingsFromProfile, ensureFleetRosterSection, ensureAutonomySection, ensureAskBackSection, ensureRecheckSection, ensureWakeGreetingSection, ensureDelegateCheckSection, ensureOneCardOneFixSection, ensureAgentIdentitySection } from './agent-scaffold.js'
...
export function startAgentProcess(name: string, opts: { fresh?: boolean } = {}): { ok: boolean; pid?: number; error?: string } {
  ...
  try {
    ...
    ensureAskBackSection(name)
    ensureRecheckSection(name)
    ensureWakeGreetingSection(name)
    ensureDelegateCheckSection(name)
