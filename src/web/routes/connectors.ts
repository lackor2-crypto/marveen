...
export async function tryHandleConnectors(ctx: RouteContext): Promise<boolean> {
  ...
  if (path === '/api/vault' && method === 'POST') {
    ...
    // at least one filled field -- an entry with nothing in it at all is still
    // rejected, since that is a mistake rather than a card.
    const normalizedFields = normalizeVaultFields(fields)
    const hasFieldValue = normalizedFields.some(f => f.value.length > 0)
    if (!id?.trim() || (!value && !hasFieldValue)) {
