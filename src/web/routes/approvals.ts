...
import { PROJECT_ROOT, MAIN_AGENT_ID, currentOwnerName, TELEGRAM_BOT_TOKEN } from '../../config.js'
import {
  ...
  listPendingApprovals, updateApprovalDescription, setApprovalTelegramMessageId,
  type Approval,
} from '../../db.js'
...
export function ensureApprovalForWaitingCard(
  ...
  try {
    ...
    const approval = createApproval({
      ...
      timeout_at: computeTimeoutAt('kanban_done', undefined),
    })
...
export async function tryHandleApprovals(ctx: RouteContext): Promise<boolean> {
  ...
  if (path === '/api/approvals' && method === 'POST') {
    let body: { agent_id?: unknown; category?: unknown; action_description?: unknown; action_payload?: unknown; noKanbanCard?: unknown; similar_reviewed?: unknown; timeout_seconds?: unknown }
    try {
    ...
    const { agent_id, category, action_description, action_payload, noKanbanCard, timeout_seconds } = body
