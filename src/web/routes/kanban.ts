...
export async function tryHandleKanban(ctx: RouteContext): Promise<boolean> {
  ...
  if (path === '/api/kanban' && method === 'POST') {
    ...
    // A kartya-letrehozas OSSZES szabalya (kotelezo cimke, kapcsolodo kartyak
    // ketiranyu belinkelese, projekt-kotes) egy helyen all: `kanban-create.ts`.
    // Azert ott, mert a Munkapad agense UGYANAZT a kaput hasznalja -- ket
    // masolatbol az egyik elobb-utobb lemaradna.
    ...
    json(res, { ok: true, id: out.id, labels: out.labels, linked: out.linked })
    return true
  }
