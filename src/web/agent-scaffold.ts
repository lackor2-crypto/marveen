...
function buildNoStrayBody(): string {
  return [
    ...
    '6. **Ebredeskor ELOSZOR a sajat felbehagyott worktree-id.** Ha a keret',
    '   kifogy es megallsz, az rendben van -- de visszatereskor a session-indito',
    '   "FELBEHAGYOTT WORKTREE-K" blokk kilistazza, melyik worktree-ben all',
    '   commitolatlan vagy landolatlan munkad (az audit-naplo szerint). Azokat',
    '   AZONNAL, minden uj munka elott fejezd be: kesz, commit, landolas. Ami mar',
    '   a mainen van es tiszta, az maradek -- toroljed kerdezes nelkul',
    '   (`scripts/agent-worktree.sh --remove <nev>`). A `scripts/land-pr.sh` a',
    '   sikeres merge utan a tiszta worktree-t amugy is maga torli (#384).',
    '',
...
function buildLandingBody(): string {
  return [
    ...
    '```',
    '',
    'Sikeres merge utan a helper a tiszta worktree-t es a lokalis branchet MAGA',
    'torli (#384; kikapcsolo: `LAND_PR_KEEP_WORKTREE=1`). Ezert a shelled',
    'munkakonyvtara megszunik: ha az eszkoz utana `getcwd`/`pwd` hibat vagy',
    'nem-nulla kilepokodot ir, az NEM a landolas hibaja -- a "MERGE-ELVE" sor a',
    'teny. Legtisztabb alhejban hivni: `(cd <worktree> && scripts/land-pr.sh "cim")`.',
    '',
...
function buildOneCardOneFixBody(): string {
  return [
    ...
    '5. **A "varakozo" (waiting) oszlop NEM felmentes a befejezes alol.** Ha egy',
    '   kartya varakozoban all, de a munka valojaban NINCS kesz, azt ugyanugy be',
    '   kell fejezni -- nem hands-off attol, hogy waiting-ben van. Nincs olyan,',
    '   hogy "ez mar waiting, ezt mar nem csinalom meg". A befejezes a kartya',
    '   MOZGATASA NELKUL is megy: komment a kartyara + a munka izolalt worktree-ben,',
    '   a kartya kozben maradhat "varakozo"-ban. Ez NEM mond ellent a visszafele-',
