...
Egyes lépések (6a, 6b) a dashboard API-t hívják; a port a `.env`-ből jön, hogy
nem-alapértelmezett porton is működjön. Ezt is EGYSZER futtasd a session elején:
...
4c. **WAITING-KOR ELLENORZES (2026-07-27-tol, ez eddig HIANYZOTT az eljarasbol)**: az audit
   ...
   SELECT id, assignee, date(updated_at,'unixepoch','localtime'), substr(title,1,60)
    ...
    ORDER BY updated_at
   ```
   (Futtatás a fenti `dbq`-val, lásd "Adatbázis-hozzáférés".)
...
4d. **KIKULDES-DETEKTOR A FRISS KARTYAKRA (2026-08-24-tol, AUDITKIKULD823 -- ez eddig HIANYZOTT,
